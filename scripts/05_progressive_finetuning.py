...
def _prepare_dataloaders(
    ...
    test_npz: Path,
    batch_size: int,
) -> tuple[MultiStation1DDataset, MultiStation1DDataset, MultiStation1DDataset, DataLoader, DataLoader, DataLoader]:
...
def _train_one_run(
    *,
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
        T_max=max(1, int(config["epochs"] // 5)),
        eta_min=float(config["lr_final"]),
    )
    ...
    save_confusion_matrix_image(
        cm=test_cm,
        labels=CLASS_NAMES,
        out_path=cm_dir / "test_cm_best_f1.png",
        title=f"Test CM - {model_key} - {target_volcano} - fold {repeat_idx:02d} - {subset_key}",
    )

    del model, best_ckpt, test_cm, optimizer, scheduler, train_loader, val_loader, test_loader
...
def main() -> None:
    ...
    for model_key in selected_models:
        ...
        spec = get_model_spec(model_key)
        if str(spec["trainer_kind"]) != "1d":
            raise ValueError(
                f"Progressive finetuning currently supports only 1d models; got {model_key}"
            )
        ...
        for target_volcano in selected_targets:
            ...
            for repeat_idx in selected_folds:
                ...
                for subset_key in available_subset_keys:
                    ...
                    rows.append(row)
                    append_row_csv(
                        output_dir / "progressive_finetuning_metrics.csv",
                        row=row,
