...
### Purpose
Builds the repeated 80/20 progressive finetuning manifests used by the target-volcano protocols.

### Main Functions
- `main()` - configures source volcanoes, target-per-class sampling, augmentation policy, and writes fold artifacts.
- `generate_cross_volcano_leave_one_out_manifests(...)` (from `utils/data_utils.py`) - legacy helper name for the per-target fold manifest generator.

### Inputs
- Prepared volcano data under `data/`.

### Outputs
- `data/prepared_data/progressive_finetuning/<VOLCANO>/fold_XX/pool_20.npz`
- `data/prepared_data/progressive_finetuning/<VOLCANO>/fold_XX/test_80.npz`
- `data/prepared_data/progressive_finetuning/<VOLCANO>/fold_XX/subsets/<subset_key>/train.npz`
- `data/prepared_data/progressive_finetuning/<VOLCANO>/fold_XX/subsets/<subset_key>/val.npz`

### Review Notes
- `test_80.npz` contains the held-out 80% evaluation split for each repeat.
- Manifests include extra metadata fields like `target_volcano`, `repeat_id`, `eval_fraction`, `pool_fraction`, and `seed`.
...
### Purpose
Evaluates model checkpoints in zero-shot mode on the held-out `test_80.npz` splits generated by script `01b_prepare_cross-volcano_data.py`.

### Main Functions
- `parse_args()` - parses experiment root, cross-data root, model filters, and output options.
- `discover_loo_target_test_paths(cross_data_root)` - resolves each target volcano to its repeat-wise `test_80.npz` paths from `progressive_finetuning`.
...
- `main()` - orchestrates the zero-shot matrix across model/fold/target/repeat combinations.

### Inputs
- Checkpoints under `results/experiments/complete_experiment/ablations/<model_key>/fold_XX/checkpoints/best_f1.pt`.
- Progressive finetuning held-out tests under `data/prepared_data/progressive_finetuning/<VOLCANO>/fold_XX/test_80.npz`.

### Outputs
- Zero-shot reports under `<experiment_root>/zero_shot_cross_volcano/`.

### Review Notes
- This script evaluates the repeated 80/20 target-volcano test splits, not the older leave-one-out manifests.
...
### Purpose
Evaluates the same zero-shot checkpoints as script `04`, but with randomized station ordering per sample.
