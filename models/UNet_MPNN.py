...
if __name__ == "__main__":
    ...
    # --- 35: forward smoke tests for a few configs --------------------------
    for name in [
        "edge_mpnn__bottleneck",
        "edge_mpnn__no_edge_feats",
        "edge_mpnn__star_topology",
    ]:
        model = UNet_MPNN(**MPNN_ABLATION_KWARGS[name])
        x = torch.randn(B, S, T)
        out = model(x)
        assert tuple(out.shape) == (B, 6, T), (name, tuple(out.shape))
        print(f"[smoke] {name}: output {tuple(out.shape)} OK")

    # --- 36: no_edge_feats has a strictly smaller message-MLP input + fewer params
    m_delta = UNet_MPNN(**MPNN_ABLATION_KWARGS["edge_mpnn__bottleneck"])
    m_none = UNet_MPNN(**MPNN_ABLATION_KWARGS["edge_mpnn__no_edge_feats"])
    in_delta = m_delta.graph_op_bottleneck.layers[0].msg_mlp[0].in_features
    in_none = m_none.graph_op_bottleneck.layers[0].msg_mlp[0].in_features
    assert in_none < in_delta, (in_none, in_delta)
    p_delta = sum(p.numel() for p in m_delta.parameters())
    p_none = sum(p.numel() for p in m_none.parameters())
    assert p_none < p_delta, (p_none, p_delta)
    print(
        f"[smoke] message-MLP in_features delta={in_delta} > none={in_none}; "
        f"params delta={p_delta} > none={p_none} OK"
    )

    # --- 37: edge-MLP weights receive non-zero gradients on the edge slice ----
    m_grad = UNet_MPNN(**MPNN_ABLATION_KWARGS["edge_mpnn__bottleneck"])
    x = torch.randn(B, S, T)
    out = m_grad(x)
    out.sum().backward()
    w = m_grad.graph_op_bottleneck.layers[0].msg_mlp[0].weight
    assert w.grad is not None, "edge-MLP weight has no gradient"
    edge_cols = w.grad[:, -m_grad.edge_attr_dim_total :]
    assert torch.isfinite(edge_cols).all(), "non-finite gradient on edge slice"
    assert edge_cols.abs().sum().item() > 0.0, "edge features unused (zero gradient)"
    print(
        "[smoke] edge-MLP edge-slice grad sum="
        f"{edge_cols.abs().sum().item():.4e} (non-zero) OK"
    )

    # --- xcorr config requires dynamic attrs ---------------------------------
    m_xcorr = UNet_MPNN(**MPNN_ABLATION_KWARGS["edge_mpnn__xcorr"])
    try:
        m_xcorr(torch.randn(B, S, T))
        raise AssertionError("expected error when xcorr attrs missing")
    except ValueError:
        print("[smoke] xcorr raises without edge_attr_dynamic OK")
    edge_attr_dynamic = torch.randn(B, m_xcorr.n_station_pairs, m_xcorr.xcorr_feat_dim)
    out = m_xcorr(torch.randn(B, S, T), edge_attr_dynamic=edge_attr_dynamic)
    assert tuple(out.shape) == (B, 6, T)
    print(f"[smoke] edge_mpnn__xcorr with dynamic attrs: {tuple(out.shape)} OK")

    # --- rsam config ----------------------------------------------------------
    m_rsam = UNet_MPNN(**MPNN_ABLATION_KWARGS["edge_mpnn__rsam"])
    out = m_rsam(torch.randn(B, S, T), rsam=torch.randn(B, S))
    assert tuple(out.shape) == (B, 6, T)
    print(f"[smoke] edge_mpnn__rsam: {tuple(out.shape)} OK")
